...
function setmetatable(src, o)
	local foo = function(t, k)
		
		return rawget(%o, k)
	end
	
	settag(src, newtag())
	settagmethod(tag(src), "index", foo)
end
...
-- local CalcH = function (self, point)
...
function PathList:MinPPoint()
	...
	for i = 1, %fn_getn(self.m_List) do 
		local path_point = self.m_List[i]
...
function PathList:Count()
	local n = %fn_getn(self.m_List)
	return n
end

function PathList:Add(path_point)
	%fn_insert(self.m_List, path_point)
end
	
function PathList:Remove(path_point)
	for i = 1, %fn_getn(self.m_List) do 
		local v_path_point = self.m_List[i]
...
function Map:CreatePoint(pxy)
	...
	if pxyb then
		local t = %PointBase:NewPoint(pxyb[1], pxyb[2], pxyb[3])
		return t
	end
...
function Map:SurroundPoints(point)
	...
	local add_tag = 1
	for i = 1, %fn_getn(%quadrant) do 
		local v = %quadrant[i]
		local temp_point = self:CreatePoint({x + v[1], y + v[2]})
		local num = %fn_getn(surround_points)
		if temp_point then
...
function Map:FindPath(start, des)
	...
	while (open_list:Count() > 0) do 
		...
		local n = %fn_getn(surround_points)
		for i = 1, n do 
			local s_point = surround_points[i]
